...
import { WalletIcon, VerifiedIcon, PackageIcon } from '../icons';
...
export const CreatorListings = ({ collections }: CreatorListingsProps) => {
  ...
  const extCollections = useMemo(
    () => collections.filter((c) => c.standard === 'ext' && !c.isLocal),
    [collections]
  );
  ...
  useEffect(() => {
    ...
    async function scan() {
      ...
      const queries = extCollections.map(async (col) => {
  ...
  }, [plugConnected, plugAccountId, extCollections]);
  ...
  const filteredNFTs = useMemo(() => {
    if (activeFilter === 'listed') return ownedNFTs.filter((n) => n.listing);
    if (activeFilter === 'unlisted') return ownedNFTs.filter((n) => !n.listing);
    return ownedNFTs;
  }, [ownedNFTs, activeFilter]);
  ...
  if (loaded && ownedNFTs.length === 0) {
    return (
      <div className="rounded-2xl border border-os-border bg-os-surface p-12 text-center">
        ...
        <p className="text-sm text-os-text-secondary">You don't own any NFTs from your collections.</p>
      </div>
    );
  }
    ...
    <div>
      <div className="flex flex-wrap items-center gap-4 mb-4">
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${
                activeFilter === tab.key
                  ...
                  : 'bg-os-card text-os-text-secondary hover:text-white'
              }`}
            >
              {tab.label}
            </button>
        ...
        <div className="flex gap-3 text-sm">
          ...
          <span className="text-os-text-secondary">&middot;</span>
...
function ListingCard({ nft, onListingChange }: {
  nft: OwnedNFT;
